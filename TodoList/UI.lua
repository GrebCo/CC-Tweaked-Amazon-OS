...
local function _emitToElements(eventName, ...)
    ...
    for i = 1, #elements do
        local e = elements[i]
        if e._visible ~= false and e._subscriptions then
...
function UI.addElement(sceneName, element)
    -- If we are currently building a layout, attach to the layout parent
    ...
    if not sceneName then error("UI.addElement: No active scene") end
...
function UI.applyPositioning(e)
    -- Skip positioning for draggable elements that have been dragged
    -- Once dragged, they maintain their manual position
    if e.draggable and e._hasBeenDragged then
        return
    end

    -- Skip positioning for children of containers - they're positioned by their parent
    -- BUT: Don't skip if we're inside a layout() call (vstack/hstack need this for dimensions)
    if e._parent and not UI._inLayout then
        return
    end
...
function UI.applyChildPositioning(parent, child)
    if not child.position then return end

    local contentX = parent.x
    local contentY = parent.y
    local contentWidth = parent.width
    local contentHeight = parent.height

    -- Account for border
    if parent.border and parent.width >= 3 and parent.height >= 3 then
        contentX = contentX + 1
        contentY = contentY + 1
    ...
    if parent.padding then
        contentX = contentX + parent.padding
        contentY = contentY + parent.padding
        contentWidth = contentWidth - (parent.padding * 2)
        contentHeight = contentHeight - (parent.padding * 2)
    end
    ...
    if pos == "center" then
        child.x = contentX + math.floor((contentWidth - childWidth) / 2)
        child.y = contentY + math.floor((contentHeight - childHeight) / 2)
    ...
    elseif pos == "topRight" then
        child.x = contentX + contentWidth - childWidth
        child.y = contentY
    elseif pos == "left" or pos == "leftCenter" then
        child.x = contentX
        child.y = contentY + math.floor((contentHeight - childHeight) / 2)
    elseif pos == "right" or pos == "rightCenter" then
        child.x = contentX + contentWidth - childWidth
        child.y = contentY + math.floor((contentHeight - childHeight) / 2)
    elseif pos == "bottomLeft" then
        child.x = contentX
        child.y = contentY + contentHeight - childHeight
    ...
    if child.xOffset then
        child.x = child.x + child.xOffset
    end
    if child.yOffset then
        child.y = child.y + child.yOffset
    end
end
...
local function hitTest(e, x, y)
    if not e or e._visible == false then return nil end
...
local function bubblePointerDown(e, x, y, button, isTouch)
    ...
    while current do
        if current._visible ~= false and current.onPress then
...
function UI._handleScroll(dir, x, y)
    ...
    local current = hit
    while current do
        if current._visible ~= false and current.onScroll then
        ...
        layout = function(self)
            ...
            for i, child in ipairs(self.children) do
                ...
                if column >= self.columns then
                    ...
                    -- Add spacing only between columns (not after last column)
                    currentX = currentX + self.columnSpacing
                end
            end
...
function UI.rectangle(opts)
    ...
    local e = {
        ...
        draw = function(self)
            ...
            -- Draw children
            for _, child in ipairs(self.children) do
                if child.draw and child._visible ~= false then
                    child:draw()
                end
            end
        end
